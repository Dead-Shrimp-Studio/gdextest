...
# 1. Build the test library and generate a disposable, self-contained fixture.
#    The framework SConscript stages the library and writes the manifest/plugin.
scons platform="$PLATFORM" target="$TARGET" tests=true -j"$(nproc)" >/dev/null

# 2. Hermetic user:// for the run (docs/testing/notes.md §3.1).
